    ...
    $(function() {
        ...
        save.click(function(e)
        {
            $.ajax({
              type: "POST",
              url: "/postrating",
              data: { comment: $('#new-review').val(), rating: $('#ratings-hidden').val(), comment: $('#new-review').val(), path : location.pathname }
            })
            .done(function( msg ) {
               getrating();
            });
...
var PriceCalc = {
    ...
    calculate: function(){
            var amount = parseFloat($('#select-choice-2').val());
            var qty = $('#slider').val();
            var service = $('#checkbox-1').parent('div').hasClass('checked') == true ? $('#checkbox-1').val() : 0;
            var total = parseFloat(service) + (amount * qty);
            var operation = $('#select-choice-2 :selected').text().trim();
            var desc = encodeURIComponent(sessionStorage.gpsName + "," + operation);
            $('#paypal').attr("href", "/order?desc=" + desc + " &qty=" + qty + "&amount=" + amount + "&service=" + service);
            $('#wishlist').attr("href", "/wishList?desc=" + desc + "&amount=" + total);
            $('#price').text("$ " + total);

    }, 
    calculate2 : function(){
        var amount = parseFloat($('#select-choice-2').val());
            var amount2 = parseFloat($('#select-choice-3').val());
            var qty = $('#slider').val();
            var service = $('#checkbox-1').parent('div').hasClass('checked') == true ? $('#checkbox-1').val() : 0;
            var total = parseFloat(service) + ((amount + amount2) * qty);
            var totalAmount = amount + amount2;
            var operation = $('#select-choice-2 :selected').text().trim();
            var desc = encodeURIComponent(sessionStorage.gpsName + "," + operation);
            $('#paypal').attr("href", "/order?desc=" + desc + " &qty=" + qty + "&amount=" + totalAmount + "&service=" + service);
            $('#wishlist').attr("href", "/wishList?desc=" + desc + "&amount=" + total);
            $('#price').text("$ " + total);
    }
    ...
    $.ajax({
                ...
                .done(function( data ) {
                    ...
                    $.each(data, function(idx, val){
                        console.log(val);
                        trs.push('<tr><td>'+ val.email +'</td><td>'+ val.date +'</td><td>' + val.comment+ '</td></tr>');
                
                    })
                 $('#tblcomment tbody').append(trs.join(' '));
                 getoverallrating();
                 });
...
function getoverallrating(){
            $.ajax({
                ...
                .done(function( data ) {
                    ...
                    if(data.rating){
                    $('#ovarallrating').attr('data-rating',data.rating);
...
function overallRating(count){

    for (var i=1; i<=count; i++) {
        $('#ovarallrating span:nth-child('+i+')').removeClass('glyphicon-star-empty');
        $('#ovarallrating span:nth-child('+i+')').addClass('glyphicon-star');
    }
}
